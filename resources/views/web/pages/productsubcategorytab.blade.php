...
<script type="text/javascript">
	...
	function fetchAllProducts(){
		 ...
		 $.ajax({
            ...
            success: function(jsonData) {
                ...
                if(jsonData.hasOwnProperty($subName) && jsonData[$subName]!=undefined){
                	for (var i =sta ; i < limit; i++){
                	var productImage='{{ asset('distweb/img/images/products')}}'+'/'+jsonData[$subName][i]['productImage'];
                $('#productscatgrid').append('<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12">'+
								'<div class="item-pro-color">'+
									'<div class="product-thumb">'+
										'<a href="/productdetail/'+jsonData[$subName][i]['id']+'" class="product-thumb-link">'+
									...
									'<div class="product-info">'+
										'<h3 class="product-title"><a href="/productdetail/'+jsonData[$subName][i]['id']+'">'+jsonData[$subName][i]['productName']+'</a></h3>'+
										'<div class="product-price">'+
										(jsonData[$subName][i]['productSku'] != ''  ? '<del><span class="prePrice"><span>'+symbolUnit+(jsonData[$subName][i]['productSku'] * currencyUnit).toFixed(2)+'</span></del>' : '')+
                               '<ins><span>'+symbolUnit+(jsonData[$subName][i]['productPrice'] * currencyUnit).toFixed(2)+'</span></ins>'+
						
										'</div>'+
										'<div class="product-extra-link">'+
											'<a href="javascript:void(0);" class="addcart-link" id="'+jsonData[$subName][i]['id']+'"><i class="fa fa-shopping-basket" aria-hidden="true"></i><span>Add to Cart</span></a>'+
											'<a href="javascript:void(0);" class="wishlist-link" id="'+jsonData[$subName][i]['id']+'"><i class="fa fa-heart" aria-hidden="true"></i><span>Wishlist</span></a>'+
										'</div>'+
									'</div>'+
								'</div>'+
							'</div>');
 ...
 //            	// console.log(jsonData);
 //            	if(jsonData.hasOwnProperty($subName) && jsonData[$subName]!=undefined){
 //                for(var i in jsonData[$subName]){
 //                	var categoryImage='{{ asset('distweb/img/images/banners')}}'+'/'+jsonData[$subName][i]['catImage'];
 //                $('#catbanner').append('<div class="banner-shop-thumb">'+
	// 										'<img src="'+categoryImage+'" alt="" />'+
	// 									'</div>'+
	// 									'<div class="banner-shop-info text-center">'+
	// 										'<h1><strong>'+jsonData[$subName][i]['catImageTitle']+'</strong></h1>'+
	// 										'<h3><strong>'+jsonData[$subName][i]['catImageGroup']+'</strong></h3>'+
	// 									'</div>');
 //            		}
 //        		}
 //            }
